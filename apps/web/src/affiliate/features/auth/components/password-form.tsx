...
interface PasswordFormProps {
  token: string;
  action: (input: unknown) => Promise<PasswordFormResult>;
  submitLabel: string;
  pendingLabel: string;
  /** Para onde mandar quem chegou com um link que não vale mais. */
  errorLink: { href: string; label: string };
}

/**
 * Criar a senha depois da aprovação e redefinir a senha esquecida são a mesma
 * tela: dois campos, um token escondido e o mesmo schema. O que muda é o action
 * que recebe, o texto do botão e para onde vai quem chegou com link vencido —
 * duplicar o formulário deixaria as duas divergirem na primeira correção.
 *
 * O token vem da query string e viaja escondido: ele é a credencial desta tela,
 * e mostrá-lo não ajudaria ninguém a usá-la.
 */
      ...
      {formError && (
        ...
        >
          ...
          <div className="text-sm font-medium text-destructive">
            <p>{formError}</p>
            <Link href={errorLink.href} className="mt-1 inline-block underline">
              {errorLink.label}
            </Link>
          </div>
        </div>
